...
def train_one_layer_perceptron(training_data, training_output, L, a, alpha, accuracy, max_epochs):
    ...
    while error > accuracy and (epoch_count < max_epochs):
        for row in range(Q):
            ...
            #backforward
            delta_o = np.reshape( d[row,:] - np.reshape(y_o, (1,M)), (M,1)) * logistic_deriv(net_o,a)
            delta_h = logistic_deriv(net_h,a) * (np.transpose(weights_HtoO) @ delta_o)
...
data = pd.read_excel('C:/Users/nuno/OneDrive - ITESO/Ciencia de Datos/IDI II/tabla_para_probar.xlsx')
training_data = data.iloc[[0,1,4,6,7,9], 0:4]
training_data = np.asarray(training_data,dtype=np.float64)
training_output = data.iloc[[0,1,4,6,7,9], -2:]
training_output = np.asarray(training_output, dtype=np.float64)
test_data = data.iloc[[2,3,5,8,10], 0:4]
